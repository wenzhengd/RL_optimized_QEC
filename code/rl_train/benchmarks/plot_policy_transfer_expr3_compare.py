"""Plot Expr1->Expr3 transfer vs fixed_zero vs Expr3-trained comparisons."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot Expr1->Expr3 transfer comparison figures.")
...
def main() -> None:
    ...
    for _, row in sorted(targets.items(), key=lambda kv: float(kv[1]["target_learned_success_rate"]), reverse=True):
        labels.append(
            f"s={float(row['scale']):g}, f={float(row['f_hz']):.0f}, g={float(row['g']):.1f}, p={float(row['p_meas']):g}"
        )
    ...
    width = 0.26
    fig, ax = plt.subplots(figsize=(11.4, 5.2), constrained_layout=True)
    ...
    ax.bar(x + width, target, width=width, color="#2a9d8f", label="Expr3-trained")
    ...
    lower_bound = float(np.min(np.concatenate([fixed_arr, transfer_arr - transfer_std_arr, target_arr])))
    upper_bound = float(np.max(np.concatenate([fixed_arr, transfer_arr + transfer_std_arr, target_arr])))
    span = max(upper_bound - lower_bound, 0.6)
    pad = max(0.12 * span, 0.08)

    ax.set_xticks(x, labels, rotation=18, ha="right")
    ax.set_ylabel("success rate (%)")
    ax.set_title("Expr1->Expr3 Transfer vs Fixed-Zero vs Expr3-Trained")
    ax.set_ylim(max(0.0, lower_bound - pad), min(100.0, upper_bound + pad))
