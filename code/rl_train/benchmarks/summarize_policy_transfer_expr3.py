"""Summarize Expr1->Expr3 transfer evaluation reports."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize Expr1->Expr3 transfer-eval JSON files.")
    parser.add_argument(
        "--input-dir",
        type=str,
        default="code/data_generated/rl_steane_tune_experiments_V2/policy_transfer_expr3",
    )
...
def main() -> None:
    ...
    reports = []
    for path in sorted(input_dir.glob("expr1_to_expr3_transfer_*.json")):
        ...
        reports.append(
            {
                ...
                "p_meas": float(target_args["steane_measurement_bitflip_prob"]),
    ...
    md_lines = ["# Expr1->Expr3 Transfer Summary", ""]
    for target_label, rows in sorted(grouped.items()):
        ...
        md_lines.append("")
        md_lines.append(
            f"- condition: `scale={sample['scale']:g}, f={sample['f_hz']:.0f}, g={sample['g']:.1f}, p_meas={sample['p_meas']:g}`"
        )
        md_lines.append(
            f"- transfer improve vs fixed_zero: `{np.mean(arr_imp):+.4f} +- {np.std(arr_imp):.4f}`"
        )
        md_lines.append(
            f"- delta success vs Expr3-trained: `{np.mean(arr_delta):+.6f} +- {np.std(arr_delta):.6f}`"
        )
        md_lines.append(
