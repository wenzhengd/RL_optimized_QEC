"""Plot delta-style summaries for Expr1->Expr3 transfer evaluation."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot Expr1->Expr3 transfer delta comparison figures.")
...
def _label(row: dict[str, float]) -> str:
    return (
        f"s={float(row['scale']):g}, f={float(row['f_hz']):.0f}, "
        f"g={float(row['g']):.1f}, p={float(row['p_meas']):g}"
    )
...
def main() -> None:
    ...
    ax.bar(
        ...
        width=width,
        color="#c8553d",
        label="transfer vs Expr3-trained: delta success (%)",
    )
    ...
    all_bounds = np.concatenate(
        [
            np.asarray(improve_mean) - np.asarray(improve_std),
            np.asarray(improve_mean) + np.asarray(improve_std),
            np.asarray(delta_success_mean) - np.asarray(delta_success_std),
            np.asarray(delta_success_mean) + np.asarray(delta_success_std),
            np.asarray([0.0]),
        ]
    )
    ...
    ax.set_ylabel("delta (%)")
    ax.set_title("Expr1->Expr3 Transfer Deltas")
